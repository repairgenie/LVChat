...
/**
 * Inline SVG icon set (feather-style: 24×24 viewBox, stroke 2, round caps,
 * currentColor). Rendered as inline <svg> so icons inherit the button colour,
 * zero network requests, and match everywhere — no emoji glyphs, no icon font.
 *
 * The JS side of the app uses an identical map (public/assets/js/icons.js and
 * messenger-web/src/icons.js); keep the names/paths in sync.
 */
 ...
 *
 * @param string      $name   Icon key (see ICON_SVG).
 * @param string      $class  Extra classes (e.g. "w-4 h-4").
 * @param string|null $title  Optional accessible title.
 */
function icon(string $name, string $class = '', ?string $title = null): string
{
    ...
    $aria = $title !== null
        ? ' role="img" aria-label="' . h($title) . '"'
        : ' aria-hidden="true"';
    return '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"' . $cls . $aria . '>' . $body . '</svg>';
}